...
class AnomalyDetector:
    ...
    def predict(self, log_entry, user_history, daily_access_count=0):
        """
        ...
        daily_access_count: The total number of data accesses today
        """
        # --- STRICT THRESHOLD GATING ---
        # If user has accessed fewer than 20 items today, do not calculate risk or detect anomalies
        if daily_access_count <= 20:
            return {
                "is_anomaly": False,
                "confidence": 0.0,
                "anomaly_score": 0.0,
        ...
        # REDUCED VOLUME WEIGHT: 0.10, HIGHER TEMPORAL/SEQUENTIAL WEIGHTS
        anomaly_score = (
            0.5 * temporal_zscore + 
            0.1 * volume_zscore + 
            0.4 * (1 - transition_prob) * 10
        )
