...
class RedisService:
    ...
    def reset_risk_score(self, user_id):
        """Reset an employee's risk score to 0 (used for false positive resolution)."""
        key = f"risk:{user_id}"
        if self.use_fake:
            self.fake_db[key] = 0.0
        else:
            self.client.set(key, 0.0)
        print(f"REDIS: Risk score reset for {user_id}")

    def increment_daily_access(self, user_id):
        # Key format: daily_access:user@email.com:2023-10-27
        date_str = time.strftime("%Y-%m-%d")
        key = f"daily_access:{user_id}:{date_str}"
        
        if self.use_fake:
            current = self.fake_db.get(key, 0)
            self.fake_db[key] = int(current) + 1
            return self.fake_db[key]
    ...
    def get_daily_access(self, user_id):
        date_str = time.strftime("%Y-%m-%d")
        key = f"daily_access:{user_id}:{date_str}"
        
        if self.use_fake:
            return int(self.fake_db.get(key, 0))
